...
epoch=100
for neuron in NEURONIOS[1:]:
    print (f"neuronios: {neuron}")
    for f in FUNCTION_GRU:
        for s in SCALER:
            for t in TIMESTEPS:
                ...
                model = Sequential()
                model.add(GRU(units=neuron, activation=f)),
                ...
                res = [1, 0, epoch, neuron, f, np.nan, s, t, scores['mae'], scores['rmse'], scores['mape']]
                resultado = resultado.append(dict(zip(resultado.columns, res)), ignore_index=True)
        resultado.to_csv("gru_1c.csv", index=False)
