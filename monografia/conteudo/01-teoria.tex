...
única e aceita \cite{wang2019defining}. Uma possível definição
...
computacional da habilidade de atingir objetivos(\cite{what-is-ai}).
...
ao extrair padrões dos dados brutos (\cite{Goodfellow-et-al-2016}).
...
partir dos dados de entrada (\cite{Goodfellow-et-al-2016}).
...
chamados de \textit{labels} ou rótulos(\cite{Goodfellow-et-al-2016}). Neste trabalho, utiliza-se aprendizado 
...
que $y \in \mathbb{R}$ (\cite{forecasting}). Seja,
...
por meio do axônio no processo de sinapse (\cite{fund_deep_learning}).
...
escrita (\cite{deeplearningbook} e \cite{zhang2021dive}). 
...
um melhor detalhamento sobre os \textit{gates} será dado abaixo.
...
camada oculta da rede neural e a entrada na mesma camada. A partir desse
