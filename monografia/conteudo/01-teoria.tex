...
Mas afinal, o que é inteligência artificial? O termo, do
...
indicador descrito na seção \ref{sec:dados} em um estado e mês e 
...
Seja, então, $\delta^i$ o vetor cujos elementos estão associados
aos neurônios da camada $i$. Como os gradientes definem a alteração nos pesos e \textit{bias}
de cada neurônio, determinam a velocidade de aprendizado dessa camada.
...
} ocorre, então, quando os gradientes das camadas iniciais ficam com valores 
muito próximos a zero. Assim, nessas camadas o ajuste nos pesos e \textit{bias}
torna-se moroso, assim como o aprendizado.

        
\subsection{Redes Neurais Recorrentes}
\label{rnn}

As redes \textit{feed forward} tradicionais não possuem
dimensão temporal. Então, ao analisar uma entrada não levam em consideração o valor
previsto para a entrada anterior, ou seja, tratam cada entrada de forma independente. 
Além disso, as informações são repassadas 
sequencialmente, dessa forma, os neurônios de uma camada recebem informação 
apenas da camada anterior, como ilustrado 
em \ref{fig:nn-trad}. Os neurônios das redes recorrentes, doutro modo, 
têm como entrada as saídas da camada anterior e o estado atual por conta dos 
ciclos ou \textit{loops} de \textit{feedback}. Por isso, a rede recorrente
leva em consideração o presente e o passado recente ao realizar uma previsão 
e apresenta bom desempenho na previsão de séries temporais ou sequências.
Em \ref{fig:rnn_loops} e \ref{fig:nn-trad} , pode-se comparar a arquitetura de uma rede \textit{feed forward}
tradicional e uma rede recorrente.
...
\begin{figure}[H]
  ...
  \begin{subfigure}{7cm}
      ...
      \caption{Redes \textit{feed forward} tradicionais}
      \label{fig:nn-trad}
  \end{subfigure}
\end{figure}

Observa-se em \ref{fig:rnn_loops}, os \textit{loops} de \textit{feedback} que 
proporcionam a captura de contexto (dimensão temporal) pelas redes recorrentes.

As redes recorrentes, então, não recebem como entrada apenas um vetor de tamanho 
fixo como as redes \textit{feed forward}. Cada \textit{input} é composto por  
uma sequência de vetores, um para cada\textit{time step}\footnote{O termo
\textit{time step} denota o tamanho da sequência que a rede recorrente receberá de entrada.}. 
Na figura \ref{fig:input-rnn}, o número
total de amostras corresponde ao campo "examples", o vetor com as 
variávies de entrada ao campo \textit{inputs} ou \textit{values per time step} (na 
figura de \textit{Recurrent Network Data}) e o tamanho das sequências consideradas
pelo modelo ao \textit{time steps}. 
...
Redes recorrentes são uma subclasse das redes neurais, contudo, há várias 
implementações disponíveis. Neste trabalho testaram-se as redes LSTM e GRU.
